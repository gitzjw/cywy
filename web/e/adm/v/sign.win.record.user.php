<?php 
	...
	$obj = C('SignWinRecord');
	$res = $obj->getSignWinRecordUserList();
?>
<section id="main" class="column">
	<article class="module width_3_quarter">
		<header>
			<h3 class="tabs_involved"><?php echo Run::req('uId');?> 奖励记录</h3>	
			<div class="submit_a_link">
				<input type="text" value="<?php echo Run::req('month');?>" placeholder="请输入月份  默认为当月" name="keyword">
				<a href="javascript:void(0);" onclick="window.location.href='?v=sign.win.record.user&uId=<?php echo Run::req('uId');?>&month='+$(this).prev().val();" class="alt_btn">搜索</a>
			</div>	
		</header>
		...
		<footer>
				...
				<?php 
					echo Run::getPageLabel($obj->getSignWinRecordUserAllCount(), $_p, Run::$PAGE_ADMIN);
				?>
				</div>
			</footer>
		</article>
