...
var _p = 1;
function defaulLoad() {
	var _params = 'action=ShopOrder&run=getShopOrderList&page=' + _p;
...
function callback(data) {
	...
	if (_d.code == '0') {
		...
		for (var o in _d.msg) {	
			...
			_html += '<a href="shop.php?tpl=v1.shop.order.detail&i='+o+'"><dl>'+
							'<dt>'+
		...
		$('#mainList').append('<p class="goods_zudd">您目前没有采购订单。</p>');
		$("#loadPage").remove();
	}
}
