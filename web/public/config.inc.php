...
define ( 'APPID', 'wx9100a963fadb1a80' );
define ( 'APPSECRET', 'bd4e35a7c4085075ef451e1eb102d360' );
define ( 'APP_MCH_ID', '1373529202' );
...
define ( 'IMG_WEBSITE', 'http://img.chongyewuyou.com:4869/' );
define ( 'APP_WEBSITE', 'http://' . $_SERVER ['HTTP_HOST'] . '/' );
define ( 'APP_NAME', 'mgs_params' );
...
class Run {
	...
	public static $SECRET_KEY = 'mgswechat';
	...
	public static function checkBrowser() {
		if (! strpos ( $_SERVER ['HTTP_USER_AGENT'], "MicroMessenger" )) {
			die ( '<html>
					...
					<body>
						<img src="http://img.chongyewuyou.com:4869/4a06f368048c265f1f1c18b99d59d237?f=png" />
						<p>请在微信客户端打开链接</p>
					</body>
				</html>' );
			exit ();
	...
	public static function getHttpRes($url, $params = NULL) {
		...
		if ($params != null) {
			$_param = '?' . http_build_query ( $params );
			$final_url .= $_param;
		}
	...
	public static function c($b = '') {
		...
		if (empty ( $user )) {
			...
			Run::show_msg ( '', 1, APP_WEBSITE . '?tpl=v1.reg' . $b );
		}
		return $user;
	}
	...
	public static function sendMessage($telNum, $info, $isRndStr = '1') {
		...
		if ($isRndStr) {
			$baseUrl = 'http://web.chongyewuyou.com/comcmc/sycmsms.php';
		} else {
			$baseUrl = 'http://web.chongyewuyou.com/comcmc/mgsnotice.php';
		}
				...
				} else //如果当前页大于左偏移
{
	...
	public static function getCityCode($code = '', $_tmp = '1') {
		$_arr = array ('100001' => '北京', '200001' => '上海', '300001' => '深圳', '400001' => '其它城市' );
		if ($_tmp == '1') {
			return isset ( $_arr [$code] ) ? $_arr [$code] : '';
		} else {
			return $_arr;
		}
	}
